...
function run(cmd, args, extra_opts = {}) {
  try {
    ...
    return { ok: false, exit: -999, out: "", err: "", ex: e.stack || e.message }
  }
}
...
L.push(
  "HEAD locale   : " +
    (h1.ok
      ? h1.out.trim()
      : "ERRORE exit=" + h1.exit + " " + (h1.err || h1.out).trim()),
)
...
L.push(
  "origin/main   : " +
    (h2.ok
      ? h2.out.trim()
      : "ERRORE exit=" + h2.exit + " " + (h2.err || h2.out).trim()),
)
...
if (h1.ok && h2.ok && h1.out.trim() === h2.out.trim())
  L.push("✅ IDENTICI = PUSH RIUSCITO. GitHub è AGGIORNATO.")
else
  L.push(
    "❌ DIVERSI = PUSH NON RIUSCITO (ancora da mandare). HEAD locale è 'ahead' di " +
      (h2.ok ? h2.out.trim() : "?") +
      ".",
  )
...
if (gl.ok) L.push(gl.out.trimEnd())
else L.push("ERR exit=" + gl.exit + " " + gl.err.trim())
...
try {
  const s = fs.statSync(vf)
  L.push("PATH: " + vf)
  L.push("BYTES: " + s.size + "  (" + Math.round(s.size / 1024) + " KB)")
  L.push("MTIME: " + s.mtime.toISOString())
} catch (e) {
  L.push("FILE NON TROVATO: " + e.message)
}
...
if (gs.ok) L.push(gs.out.trimEnd())
else L.push("ERR exit=" + gs.exit + " " + gs.err.trim())
...
if (rv.ok) L.push(rv.out.trimEnd())
else L.push("ERR exit=" + rv.exit + " " + rv.err.trim())
