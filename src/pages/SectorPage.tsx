...
const steps = [
  { n: "01", label: "Sopralluogo", desc: "Visitiamo il tuo spazio, ascoltiamo le esigenze e prendiamo le misure." },
  { n: "02", label: "Progettazione", desc: "Sviluppiamo il progetto con render 3D e disegni tecnici esecutivi." },
  { n: "03", label: "Realizzazione", desc: "Costruiamo nel nostro laboratorio con materiali selezionati." },
  { n: "04", label: "Installazione", desc: "Consegna e posa in opera nei tempi pattuiti, chiavi in mano." },
];
    ...
    return (
      <div className="min-h-screen flex items-center justify-center pt-20 bg-[#F7F5F0]">
        <div className="text-center">
          <h1 className="font-display text-3xl text-[#1A1A18] mb-4">Settore non trovato</h1>
          <Link to="/" className="text-[#1B4332] text-sm hover:underline">Torna alla home →</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-[#F7F5F0]">
      ...
      <section className="relative h-[70vh] flex items-end pb-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-10 w-full">
          <Link to="/" className="text-white/60 text-xs mb-6 inline-block hover:text-white transition-colors">
            ← Home
          </Link>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          <div>
            <h2 className="font-display text-3xl lg:text-4xl font-light text-[#1A1A18] mb-6">
              Cosa realizziamo per<br />
              <em className="text-[#1B4332]">{sector.label.toLowerCase()}</em>
            </h2>
            <p className="text-[#4A4A46] leading-relaxed mb-8">{sector.description}</p>
            <Link
              to="/preventivo"
      ...
      {sectorProjects.length > 0 && (
        <section className="py-16 bg-[#1A1A18]">
          <div className="max-w-7xl mx-auto px-6 lg:px-10">
            <div className="mb-12">
              <span className="text-[#888580] text-xs tracking-widest uppercase font-semibold">Realizzazioni</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {sectorProjects.map((p) => (
                ...
                >
                  ...
                  <div className="p-6">
                    <h3 className="font-display text-lg font-light text-white mb-1">{p.title}</h3>
      ...
      <section className="py-20 max-w-7xl mx-auto px-6 lg:px-10">
        <div className="text-center mb-14">
          <span className="text-[#888580] text-xs tracking-widest uppercase font-semibold">Il processo</span>
          <h2 className="font-display text-3xl lg:text-4xl font-light text-[#1A1A18] mt-2">Come lavoriamo</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((s) => (
            <div key={s.n} className="border-t-2 border-[#1B4332] pt-6">
              <span className="font-mono text-[#B5965A] text-sm mb-3 block">{s.n}</span>
              <h3 className="font-display text-xl font-medium text-[#1A1A18] mb-2">{s.label}</h3>
      ...
      <section className="py-16 bg-[#EAE7E0]">
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {[
              ["Esperienza settoriale", "Conosciamo le esigenze specifiche del settore per proporti le soluzioni più efficaci."],
              ["Personalizzazione totale", "Nessun catalogo, nessun compromesso. Ogni arredo nasce sulle tue misure e il tuo stile."],
              ["Assistenza post-vendita", "Siamo presenti anche dopo la consegna per qualsiasi manutenzione o adeguamento."],
            ].map(([t, d]) => (
              <div key={t} className="bg-white p-6 border-l-2 border-[#1B4332]">
                <h3 className="font-display text-lg font-medium text-[#1A1A18] mb-2">{t}</h3>
      ...
      <section className="py-20 bg-[#1B4332] text-center">
        <div className="max-w-2xl mx-auto px-6">
          ...
          <p className="text-white/70 mb-8">
            Raccontaci il tuo progetto: ti ricontatteremo entro 24 ore per fissare un sopralluogo gratuito.
          </p>
          <Link
