...
type FormState = {
  titolo: string; settore: string; cliente: string; citta: string; anno: string; stato: string; descrizione: string; evidenza: boolean;
};

const emptyForm: FormState = { titolo: "", settore: "", cliente: "", citta: "", anno: "2025", stato: "bozza", descrizione: "", evidenza: false };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#888580] text-sm mt-0.5">{projects.length} progetti totali</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-white border border-[#DDD9D0] p-6 mb-6">
          <h2 className="font-display text-xl font-light text-[#1A1A18] mb-5">Nuovo progetto</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {(["titolo", "cliente", "citta"] as const).map((k) => (
              <div key={k}>
                <label className="block text-xs text-[#888580] uppercase tracking-wide mb-1">{k}</label>
                <input
                  type="text"
                  value={form[k]}
            ...
            <div>
              <label className="block text-xs text-[#888580] uppercase tracking-wide mb-1">Settore</label>
              <select
                value={form.settore}
            ...
            <div>
              <label className="block text-xs text-[#888580] uppercase tracking-wide mb-1">Anno</label>
              <input
                type="number"
                value={form.anno}
            ...
            <div>
              <label className="block text-xs text-[#888580] uppercase tracking-wide mb-1">Stato</label>
              <select
                value={form.stato}
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs text-[#888580] uppercase tracking-wide mb-1">Descrizione</label>
              <textarea
                rows={3}
            ...
            <div className="sm:col-span-2 flex items-center gap-3">
              ...
              />
              <label htmlFor="evidenza" className="text-sm text-[#4A4A46]">Mostra in evidenza nella home</label>
            </div>
          </div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        ...
        <span className="text-[#888580] text-xs self-center">{filtered.length} risultati</span>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[#F7F5F0] text-[#888580] text-xs uppercase tracking-wide border-b border-[#DDD9D0]">
              ...
              <th className="text-left px-5 py-3 hidden md:table-cell">Settore</th>
              <th className="text-left px-5 py-3 hidden lg:table-cell">Cliente</th>
              ...
              <tr key={p.id} className="border-t border-[#EAE7E0] hover:bg-[#F7F5F0] transition-colors">
                <td className="px-5 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-medium text-[#1A1A18]">{p.title}</span>
                  </div>
                </td>
                <td className="px-5 py-3 hidden md:table-cell text-[#4A4A46] text-xs">{p.sector}</td>
                <td className="px-5 py-3 hidden lg:table-cell text-[#4A4A46] text-xs">{p.client || "—"}</td>
                <td className="px-5 py-3 hidden sm:table-cell text-[#888580] text-xs">{p.year}</td>
                <td className="px-5 py-3">
                  <span className={`text-xs px-2.5 py-1 font-medium rounded-full ${statusColor[p.stato] || "bg-gray-100 text-gray-600"}`}>
                    {p.stato}
                  </span>
                </td>
                <td className="px-5 py-3">
