...
export default function AdminSettings() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="font-display text-3xl font-light text-[#1A1A18]">Impostazioni</h1>
      </div>
        ...
        {(["generale", "utenti"] as const).map((t) => (
          <button
            ...
            className={`px-5 py-2.5 text-sm font-medium transition-all border-b-2 -mb-px ${
              activeTab === t ? "border-[#1B4332] text-[#1B4332]" : "border-transparent text-[#888580] hover:text-[#1A1A18]"
            }`}
          >
      ...
      {activeTab === "generale" && (
        <div className="max-w-2xl bg-white border border-[#DDD9D0] p-6 space-y-6">
          {[
            { label: "Nome azienda", defaultVal: "ArtigianaLegno Srl", type: "text" },
            { label: "P.IVA", defaultVal: "02345678901", type: "text" },
            { label: "Telefono sito", defaultVal: "+39 051 234 5678", type: "tel" },
            { label: "Email contatti", defaultVal: "info@artigianalegno.it", type: "email" },
            { label: "Città / indirizzo footer", defaultVal: "Via dell'Artigiano, 14 – Bologna", type: "text" },
          ...
          <div>
            <label className="block text-xs text-[#888580] uppercase tracking-wide mb-1.5">Testo footer</label>
            <textarea
              rows={3}
        ...
        <div className="max-w-3xl">
          <div className="bg-white border border-[#DDD9D0] overflow-hidden mb-4">
            <table className="w-full text-sm">
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u.id} className="border-t border-[#EAE7E0] hover:bg-[#F7F5F0]">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-[#1A1A18]">{u.nome}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3 hidden sm:table-cell text-[#888580] text-xs">{u.email}</td>
                    <td className="px-5 py-3">
                      <span className={`text-xs px-2.5 py-1 font-medium rounded-full ${roleColor[u.ruolo]}`}>
                        {u.ruolo}
                      </span>
                    </td>
                    <td className="px-5 py-3">
                      <button className="text-xs text-[#888580] hover:text-[#1B4332] mr-3 transition-colors">Modifica</button>
                      {u.ruolo !== "admin" && (
                        <button className="text-xs text-red-400 hover:text-red-600 transition-colors">Rimuovi</button>
                      )}
                    </td>
                  </tr>
                ))}
