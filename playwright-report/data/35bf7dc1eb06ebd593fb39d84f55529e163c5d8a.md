...
- Name: navigation.e2e.spec.ts >> Navigation >> should scroll to sections on home page
- Location: e2e\navigation.e2e.spec.ts:117:7
...
Expected: visible
Error: strict mode violation: locator('#hero') resolved to 2 elements:
    1) <section id="hero" data-bg="#1A1A2E">…</section> aka locator('#hero').first()
    2) <section id="hero" aria-label="Sezione introduttiva" class="relative min-h-[100svh] overflow-hidden bg-[#1A1A2E] flex items-center">…</section> aka getByRole('region', { name: 'Sezione introduttiva' })

Call log:
  - Expect "toBeVisible" locator('#hero') with timeout 5000ms
