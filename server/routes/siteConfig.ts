...
// GET showroom configuration (public endpoint)
router.get('/showroom', async (req: Request, res: Response) => {
  // Security: Ensure only GET method is accepted
  if (req.method !== 'GET') {
    return res.status(405).json({
      error: 'Method not allowed'
  ...
  try {
    ...
    const showSoldProducts = config ? config.value === 'true' : false;

    res.json({ showSoldProducts });
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch showroom configuration' });
  }
});

// POST showroom configuration (admin only)
router.post('/showroom', requireAdmin, async (req: Request, res: Response) => {
  // Security: Ensure only POST method is accepted
  if (req.method !== 'POST') {
    return res.status(405).json({
      error: 'Method not allowed'
  ...
  try {
    const { showSoldProducts } = req.body;

    // Validate input
    if (typeof showSoldProducts !== 'boolean') {
      return res.status(400).json({
        error: 'showSoldProducts must be a boolean'
      });
    }

    // Update or create configuration
    await SiteConfig.findOneAndUpdate(
      { key: 'showroom_show_sold_products' },
      {
        key: 'showroom_show_sold_products',
        value: showSoldProducts ? 'true' : 'false',
        description: 'Mostra prodotti venduti nel frontend pubblico',
        isSensitive: false,
        updatedAt: new Date()
      },
      { upsert: true, new: true }
    ...
    res.json({ success: true, showSoldProducts });
  } catch (error) {
    console.error('Error saving showroom configuration:', error);
    res.status(400).json({ error: 'Failed to save showroom configuration' });
  }
});
